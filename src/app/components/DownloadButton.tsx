...
const DownloadButton: React.FC<DownloadButtonProps> = ({ id, type, releaseYear }) => {
  const t = useTranslations('DownloadButton'); // Translation hook

  return (
    <Link href={`/processing?id=${id}&type=${type}&year=${releaseYear !== "unknown" ? releaseYear : "unknown"}`}>
