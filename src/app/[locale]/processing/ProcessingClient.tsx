...
const ProcessingClient: React.FC<ProcessingClientProps> = ({
  ...
  const locale = useLocale(); // Use useLocale() to handle locale
  ...
  const handleContinue = () => {
    if (isButtonEnabled) {
      // Redirect to the `proceed` page, NOT directly to the download page
      router.push(`/${locale}/proceed?id=${id}&type=${type}&year=${releaseYear}`);
    }
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center">
      ...
      >
        {isButtonEnabled ? proceedToDownload : `${pleaseWait} ${countdown} ${seconds}`}
      </button>
